...
/datum/preferences
	...
	var/datum/statpack/statpack	= new /datum/statpack/alizeria/none // LETHALSTONE EDIT: the statpack we're giving our char instead of racial bonuses
	...
	var/static/datum/patron/default_patron = /datum/patron/inhument/zizot
...
/datum/preferences/proc/set_new_race(datum/species/new_race, user)
	...
	if(user)
		...
		to_chat(user, "<font color='red'>Роли сброшены.</font>")
...
//	dat += "<a href='?_src_=prefs;preference=tab;tab=3' [current_tab == 3 ? "class='linkOn'" : ""]>Keybinds</a>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<a href='?_src_=prefs;preference=job;task=menu'>Выбор роли</a>"
			dat += "</td>"

			dat += "<td style='width:33%;text-align:right'>"
			dat += "<a href='?_src_=prefs;preference=keybinds;task=menu'>Назначения клавиш</a>"
			dat += "</td>"
			dat += "</tr>"
...
//			dat += "<a href='?_src_=prefs;preference=toggle_random;random_type=[RANDOM_NAME_ANTAG]'>When Antagonist: [(randomise[RANDOM_NAME_ANTAG]) ? "Yes" : "No"]</a>"
			...
			dat += "<BR>"
			dat += "<b>Раса:</b> <a href='?_src_=prefs;preference=species;task=input'>[pref_species.base_name]</a>[spec_check(user) ? "" : " (!)"]<BR>"
			dat += "<b>Подраса:</b> <a href='?_src_=prefs;preference=subspecies;task=input'>[pref_species.sub_name]</a>[spec_check(user) ? "" : " (!)"] <a href='?_src_=prefs;preference=racehelp;task=input'>[pref_species.psydonic ? "<font color='#1cb308'>𝕂</font>" : "<font color='#aa0202'>Ʞ</font>"]</a><BR>"
			if(pref_species.use_titles)
				var/display_title = selected_title ? selected_title : "Нету"
				dat += "<b>Тип расы:</b> <a href='?_src_=prefs;preference=race_title;task=input'>[display_title]</a><BR>"
			dat += "<b>Происхождение:</b> <a href='?_src_=prefs;preference=origin;task=input'>[virtue_origin]</a><BR>"
			if(agevetted)
				dat += "<b>Семья:</b> <a href='?_src_=prefs;preference=family'>[family ? family : "Нету"]</a><BR>"
				if(family != FAMILY_NONE)
					var/spousename = "Предпочитаемый супруг"
					if(family == FAMILY_PARTIAL)
						spousename = "Предпочтительный брат/сестра"
					dat += "<b>[spousename]:</b> <a href='?_src_=prefs;preference=setspouse'>[setspouse ? setspouse : "Нету"]</a><BR>"
					if(family == FAMILY_NEWLYWED || family == FAMILY_FULL)
						dat += "<b>Предпочитаемый пол:</b> <a href='?_src_=prefs;preference=gender_choice'>[gender_choice ? gender_choice : "Любой"]</a><BR>"
						var/species_text
						if(xenophobe_pref == 1)
							species_text = "<font color='#FFA500'>Только расы</font>"
						else if(xenophobe_pref == 2)
							species_text = "<font color='#aa0202'>Только подрасы</font>"
						else
							species_text = "<font color='#1cb308'>Без ограничений</font>"
						dat += "<b>Ограничение рас:</b> <a href='?_src_=prefs;preference=species_choice'>[species_text]</a><BR>"
			else
				dat += "<b>Вам нужно быть <font color='#31acd5'>Доверенным</font> что бы пользоваться Headshot.</b><BR>"

			var/datum/language/selected_lang
			var/lang_output = "Нету"
			if(ispath(extra_language, /datum/language))
				selected_lang = extra_language
				lang_output = initial(selected_lang.name)

			dat += "[virtue_origin.extra_language ? "<b>Дополнительный язык: </b>" : "<s><b>Дополнительный язык:</b></s> "]<a href='?_src_=prefs;preference=extra_language;task=input'>[lang_output]</a><BR>"

			// LETHALSTONE EDIT BEGIN: add statpack selection
			dat += "<b>Характеристики:</b> <a href='?_src_=prefs;preference=statpack;task=input'>[statpack.name]</a><BR>"
			dat += "<BR>"
...
//			dat += "<a href='?_src_=prefs;preference=toggle_random;random_type=[RANDOM_SPECIES]'>Always Random Species: [(randomise[RANDOM_SPECIES]) ? "Yes" : "No"]</A><br>"
			...
			if(!(AGENDER in pref_species.species_traits))
				...
				if(gender == MALE)
					...
					dispGender = "Иное"
				dat += "<b>Тело:</b> <a href='?_src_=prefs;preference=gender'>[dispGender]</a><BR>"
			...
			if((LAMIAN_TAIL in pref_species.species_traits))
				...
				dat += "<b>Тип хвоста:</b> <a href='?_src_=prefs;preference=tail_type;task=input'>[name]</a><BR>"
				dat += "<b>Цвет хвоста:</b><span style='border: 1px solid #161616; background-color: #[tail_color];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=tail_color;task=input'>Изменить</a><BR>"
				dat += "<b>Цвет отметок:</b><span style='border: 1px solid #161616; background-color: #[tail_markings_color];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=tail_markings_color;task=input'>Изменить</a><BR>"

			// LETHALSTONE EDIT BEGIN: add voice type prefs
			//dat += "<b>Голос</b>: <a href='?_src_=prefs;preference=voicetype;task=input'>[voice_type]</a><BR>"
			// LETHALSTONE EDIT END
...
//			dat += "<b><a href='?_src_=prefs;preference=name;task=random'>Random Name</A></b><BR>"
			dat += "<b>Особенность:</b> <a href='?_src_=prefs;preference=virtue;task=input'>[virtue]</a><BR>"
			if(statpack.name == "Virtuous")
				dat += "<b>Вторая особенность:</b> <a href='?_src_=prefs;preference=virtuetwo;task=input'>[virtuetwo]</a><BR>"
...
//			-----------START OF BODY TABLE-----------
			...
			dat += "<b>Обновлять цвет элементов тела с обновлением основного цвета кожи:</b> <a href='?_src_=prefs;preference=update_mutant_colors;task=input'>[update_mutant_colors ? "Да" : "Нет"]</a><BR>"
			var/use_skintones = pref_species.use_skintones
			if(use_skintones && !(LAMIAN_TAIL in pref_species.species_traits))
				if(pref_species.id != "lupian")
					dat += "<b>[pref_species.skin_tone_wording]: </b><a href='?_src_=prefs;preference=skin_color_ref_list;task=input'>(?)</a> <a href='?_src_=prefs;preference=s_tone;task=input'>Изменить</a>"
				else
					dat += "<b>[pref_species.skin_tone_wording]: </b><a href='?_src_=prefs;preference=s_tone;task=input'>Изменить</a>"
				dat += "<br>"
			...
			if((MUTCOLORS in pref_species.species_traits) && !(LAMIAN_TAIL in pref_species.species_traits) && !(HARPY in pref_species.species_traits) || (MUTCOLORS_PARTSONLY in pref_species.species_traits) && !(LAMIAN_TAIL in pref_species.species_traits) && !(HARPY in pref_species.species_traits))
				...
				dat += "<b>Изменяемый цвет #1:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color;task=input'>Изменить</a><BR>"
				dat += "<b>Изменяемый цвет #2:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor2"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color2;task=input'>Изменить</a><BR>"
				dat += "<b>Изменяемый цвет #3:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor3"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color3;task=input'>Изменить</a><BR>"

			if((LAMIAN_TAIL in pref_species.species_traits))

				dat += "<b>Цвет кожи/чешуи #1:</b><a href='?_src_=prefs;preference=skin_color_ref_list;task=input'>(?)</a><span style='border: 1px solid #161616; background-color: #[features["mcolor"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=skin_choice_pick;task=input'>Изменить</a><BR>"
				dat += "<b>Цвет элемента #1:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor2"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color2;task=input'>Изменить</a><BR>"
				dat += "<b>Цвет элемента #2:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor3"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color3;task=input'>Change</a><BR>"

			if(HARPY in pref_species.species_traits)

				dat += "<b>Цвет кожи/чешуи #1:</b><a href='?_src_=prefs;preference=skin_color_ref_list;task=input'>(?)</a><span style='border: 1px solid #161616; background-color: #[features["mcolor"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=skin_feathers_pick;task=input'>Изменить</a><BR>"
				dat += "<b>Цвет элемента #1:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor2"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color2;task=input'>Изменить</a><BR>"
				dat += "<b>Цвет элемента #2:</b><span style='border: 1px solid #161616; background-color: #[features["mcolor3"]];'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color3;task=input'>Изменить</a><BR>"

			dat += "<b>Тон голоса: </b><a href='?_src_=prefs;preference=voice;task=input'>Изменить</a>"
			...
			dat += "<br><b>Размер тела:</b><a href='?_src_=prefs;preference=body_size;task=input'>[(features["body_size"] * 100)]%</a>"
			dat += "<br><b>Отметки:</b> <a href='?_src_=prefs;preference=markings;task=menu'>Изменить</a>"
			...
			dat += "<br><b>[(length(flavortext) < MINIMUM_FLAVOR_TEXT) ? "<font color = '#802929'>" : ""]Описание персонажа:[(length(flavortext) < MINIMUM_FLAVOR_TEXT) ? "</font>" : ""]</b><a href='?_src_=prefs;preference=formathelp;task=input'>(?)</a><a href='?_src_=prefs;preference=flavortext;task=input'>Изменить</a>"

			dat += "<br><b>[(length(ooc_notes) < MINIMUM_OOC_NOTES) ? "<font color = '#802929'>" : ""]OOC Заметки:[(length(ooc_notes) < MINIMUM_OOC_NOTES) ? "</font>" : ""]</b><a href='?_src_=prefs;preference=formathelp;task=input'>(?)</a><a href='?_src_=prefs;preference=ooc_notes;task=input'>Изменить</a>"

			// Rumours / Gossip
			dat += "<br><b>Слухи:</b> <a href='?_src_=prefs;preference=rumour;task=input'>Изменить</a>"
			dat += "<br><b>История дворянства:</b> <a href='?_src_=prefs;preference=gossip;task=input'>Изменить</a>"
		...
		if(2) //OOC Preferences
			...
			if(user.client)
				...
				if(unlock_content || check_rights_for(user.client, R_ADMIN))
					dat += "<b>OOC Color:</b> <span style='border: 1px solid #161616; background-color: [ooccolor ? ooccolor : GLOB.normal_ooc_colour];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=ooccolor;task=input'>Изменить</a><br>"
...
//							dat += "| Default: [default_keys.Join(", ")]"
	...
	dat += "<b>Эмбиент:</b> <a href='?_src_=prefs;preference=ambientocclusion'>[ambientocclusion ? "Включен" : "Отключен"]</a><br>"
	dat += "</td>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("name")
					var/new_name = tgui_input_text(user, "Как вас будут называть?", "ЛИЧНОСТЬ", encode = FALSE)
					if(new_name)
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("age")
					var/new_age = tgui_input_list(user, "Выберите возраст вашего персонажа.", "ВОЗРАСТ", pref_species.possible_ages)
					if(new_age)
						age = new_age
						var/list/hairs
				...
				if ("statpack")
					...
					var/statpack_input = tgui_input_list(user, "Каковы сильные стороны вашего персонажа?", "ХАРАКТЕРИСТИКИ", statpacks_available, statpack)
					if (statpack_input)
				...
				if ("pronouns")
					var pronouns_input = tgui_input_list(user, "Выберите местоимение для вашего персонажа.", "МЕСТОИМЕНИЯ", GLOB.pronouns_list)
					if(pronouns_input)
				...
				if ("voicetype")
					var voicetype_input = tgui_input_list(user, "Выберите тип озвучки вашего персонажа.", "ГОЛОС", GLOB.voice_types_list)
					if(voicetype_input)
					...
					if(virtue_origin.uniquefaith)
						...
						for(var/path as anything in GLOB.preference_faiths)
							...
							if(!faith.name)
								continue
							faiths_named[faith.name] = faith
					var/faith_input = tgui_input_list(user, "Во что верует ваш персонаж?", "ВЕРА", faiths_named)
					if(faith_input)
				...
				if("patron")
					...
					var/god_input = tgui_input_list(user, "Кто именно покровительствует вашему персонажу?", "ПОКРОВИТЕЛЬ", patrons_named)
					if(god_input)
				...
				if("combat_music") // if u change shit here look at /client/verb/combat_music() too
					...
					var/track_select = tgui_input_list(user, "Во время битвы будет играть:", "БОЕВАЯ МУЗЫКА", GLOB.cmode_tracks_by_name, combat_music?.name)
					if(track_select)
				...
				if("extra_language")
					...
					var/chosen_language = tgui_input_list(user, "Выберите дополнительный язык для своего персонажа.", "ДОПОЛНИТЕЛЬНЫЙ ЯЗЫК", choices)
					if(chosen_language)
						to_chat(user, "<span class='notice'>Дополнительного языка не будет если роль, раса или происхождение не подразумевают под собой его наличие.</span>")
						if(chosen_language == "Нету")
							extra_language = "Нету"
						else
				...
				if("race_title")
					...
					for(var/A in titles)
						...
						choices += list(A)
					if(user?.client)
						var/result = tgui_input_list(user, "Кем именно будет ваш персонаж?", "ТИП РАСЫ", choices)
				...
				if("racehelp")
					var/list/dat = list()
					dat += "<font color='#1cb308'>𝕂</font> - символ, обозначающий <b>благородную</b> расу, представители которой в большинстве своём обладают дворянским титулом.<br>"
					dat += "Представители этой расы чтят <b>кодекс</b> и никогда не смеют его нарушать по собственной воле. <i>По крайней мере в это принято верить...</i><br>"
					dat += "<font color='#aa0202'>Ʞ</font> - символ, обозначающий <b>отвергнутую</b> расу, что была по какой-либо причине опозорена и лишена достоинства.<br>"
					dat += "<b>Кодекс</b> для представителей подобной расы ничего не значит. Они могут спокойно ему не следовать, ибо он к ним не относится. <i>Однако в мире есть и те, кто думает иначе...</i>"
				...
				if("species")
					...
					var/result = tgui_input_list(user, "Какой расы будет ваш персонаж?", "РАСА", species)
				...
				if("subspecies")
					...
					var/result = tgui_input_list(user, "Какой подрасы будет ваш персонаж?", "ПОДРАСА", species)
				...
				if("virtue")
					...
					var/result = tgui_input_list(user, "Чем будет обладать ваш персонаж?", "ОСОБЕННОСТЬ",virtue_choices)
					...
					for (var/path as anything in GLOB.virtues)
						...
						if (istype(V, /datum/virtue/origin/racial))
							if(!(pref_species.type in V.races))
								continue
						virtue_choices[V.name] = V
					var/result = tgui_input_list(user, "Откуда ваш персонаж родом?", "ПРОИСХОЖДЕНИЕ",virtue_choices)
				...
				if("charflaw")
					...
					var/result = tgui_input_list(user, "Что за слабость будет у вашего персонажа?", "НЕДОСТАТОК",coom)
					if(result)
						result = coom[result]
...
*/
				if("s_tone")
					var/listy = pref_species.get_skin_list()
					var/new_s_tone = tgui_input_list(user, "Выберите цвет кожи вашего персонажа.", "ЦВЕТ КОЖИ", listy)
					if(new_s_tone)
				...
				if("char_accent")
					var/selectedaccent = tgui_input_list(user, "Выберите акцент для вашего персонажа.", "АКЦЕНТ", GLOB.character_accents)
					if(selectedaccent)
			...
			switch(href_list["preference"])
				...
				if("domhand")
					if(domhand == 1)
						...
						domhand = 1
				if("family")
					var/list/famtree_options_list = list(FAMILY_NONE, FAMILY_PARTIAL, FAMILY_NEWLYWED, "ОБЪЯСНИТЕ МНЕ!")
					if(age != AGE_ADULT)
						famtree_options_list = list(FAMILY_NONE, FAMILY_PARTIAL, FAMILY_NEWLYWED, FAMILY_FULL, "ОБЪЯСНИТЕ МНЕ!")
					var/new_family = tgui_input_list(user, "SELECT YOUR HERO'S BOND", "BLOOD IS THICKER THAN WATER", famtree_options_list, family)
					if(new_family == "ОБЪЯСНИТЕ МНЕ!")
						to_chat(user, span_purple("\
						--<b>[FAMILY_NONE]</b> отключит семьи для вас.<br>\
						--<b>[FAMILY_PARTIAL]</b> делает вас одним из потомков дома. Если ваш персонаж взрослый или старый - вы будете иметь шанс стать тётей или дядей.<br>\
						--<b>[FAMILY_NEWLYWED]</b> назначает вам супруга(-у), не добавляя вас в семью.<br>\
						--<b>[FAMILY_FULL]</b> будет предпринята попытка назначить вас матриархом или патриархом одного из местных домов."))

					else if(new_family)
						family = new_family
						setspouse = null
						gender_choice = DIFFERENT_GENDER
						xenophobe_pref = 1
				...
				if("gender_choice")
					...
					if(pronouns == THEY_THEM || pronouns == IT_ITS)
						...
						var/list/gender_choice_option_list = list(DIFFERENT_GENDER)
				...
				if("schizo_voice")
					...
					if(toggles & SCHIZO_VOICE)
						to_chat(user, "<span class='warning'>Теперь вы - голос.\n\
										Будучи голосом вы будете получать вопросы от игроков во время медитаций. Эти вопросы должны быть связаны исключительно с игровыми механиками.\n\
										Старайтесь воздерживаться от шуток или неправильных ответов, какой бы вопрос вам не задали.</span>")
					else
						to_chat(user, span_warning("Вы больше не голос."))
