...
/datum/component/martyrweapon/proc/on_equip(datum/source, mob/user, slot)
	if(!allow_all)
		if(ishuman(user))
			...
			if(!J.title == "Guardian of Ten" && !J.title == "Priest")		//Can't be a Martyr if you're not a Martyr. Or a Priest.
...
/datum/job/roguetown/martyr
	title = "Guardian of Ten"
	department_flag = CHURCHMEN
	faction = "Station"
	tutorial = "Ты - старовер, что дал клятву вместе со жрецом недопустить кровопролития из-за низменных желаний. Священные войны длились веками, однако сейчас главную угрозу представляют люди, что хотят заполучить власть путём лишения жизни тех, кто за эту власть биться вовсе не хочет."
	allowed_sexes = list(MALE, FEMALE)
	allowed_races = ALIZ_NOBLE_RACES // fuck you golen/doll martyr because it's FUNNY
...
/datum/outfit/job/martyr/basic/pre_equip(mob/living/carbon/human/H)
	...
	beltr = /obj/item/storage/keyring/alizeria/ten
...
/obj/item/rogueweapon/sword/long/martyr/attack_hand(mob/user)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		var/datum/job/J = SSjob.GetJob(H.mind?.assigned_role)
		if(J.title == "Priest" || J.title == "Guardian of Ten")
			return ..()
