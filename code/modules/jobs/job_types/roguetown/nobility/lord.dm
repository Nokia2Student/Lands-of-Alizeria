...
/datum/job/roguetown/lord
	...
	tutorial = "Ты - наместник земель Ализерии, отправленный лично короной ради сохранения её влияния на данных территориях. Твоим сюзереном является королевство Монфор, однако в текущей обстановке на острове - это не так важно. Морские штормы не дают тебе возможности как-либо связаться с короной, и ко всему прочему коренное население острова неочень то и желает тебя поддерживать. Однако, есть и кое-что хорошее. Жандармы, что прибыли вместе с тобой - пускай и обязались защищать народ, но защищать тебя они желают в первую очередь, так как именно от тебя и других дворян зависит поступающее к ним золото. Если золота не будет, что же... Тебе останется лишь молиться за свою безопасность."
	whitelist_req = FALSE
	min_pq = 0
	max_pq = null
	allowed_patrons = CODEX
...
/datum/outfit/job/lord
	...
	beltl = /obj/item/storage/keyring/alizeria/lady
...
/obj/effect/proc_holder/spell/self/grant_title
	name = "Присвоить титул"
	desc = "Присвойте кому-нибудь почётный титул... Или позорный."
...
/obj/effect/proc_holder/spell/self/grant_title/cast(list/targets, mob/user = usr)
	. = ..()
	var/granted_title = input(user, "Какой титул вы хотите дать?", "[name]") as null|text
	...
	if(!length(recruitment))
		to_chat(user, span_warning("Рядом нет никого достойного этого."))
		return
	var/inputty = input(user, "Выберите достойного!", "[name]") as anything in recruitment
	if(inputty)
		...
		if(!QDELETED(recruit) && (recruit in get_hearers_in_view(title_range, user)))
			...
			to_chat(user, span_warning("Провал!"))
	else
		to_chat(user, span_warning("Достойный отказался."))
...
/obj/effect/proc_holder/spell/self/grant_title/proc/village_idiotify(mob/living/carbon/human/recruit, mob/living/carbon/human/recruiter, granted_title)
	...
	if(GLOB.lord_titles[recruit.real_name])
		recruiter.say("Своей великой властью я лишаю тебя, [uppertext(recruit.name)], титула [uppertext(GLOB.lord_titles[recruit.real_name])]!")
		GLOB.lord_titles -= recruit.real_name
		return FALSE
	recruiter.say("Своей великой властью я дарую тебе, [uppertext(recruit.name)], титул [uppertext(granted_title)]!")
...
/obj/effect/proc_holder/spell/self/grant_nobility
	name = "Даровать дворянство"
	desc = "Сделайте кого-нибудь дворянином или лишите его этого."
	overlay_state = "recruit_titlegrant"
	antimagic_allowed = TRUE
	recharge_time = 100
...
/obj/effect/proc_holder/spell/self/grant_nobility/cast(list/targets, mob/user = usr)
	...
	for(var/mob/living/carbon/human/village_idiot in (get_hearers_in_view(nobility_range, user) - user))
		...
		recruitment[village_idiot.name] = village_idiot
	if(!length(recruitment))
		to_chat(user, span_warning("Рядом нет никого достойного."))
		return
	var/inputty = input(user, "Выберите достойного!", "[name]") as anything in recruitment
	if(inputty)
		var/mob/living/carbon/human/recruit = recruitment[inputty]
...
/obj/effect/proc_holder/spell/self/grant_nobility/proc/grant_nobility(mob/living/carbon/human/recruit, mob/living/carbon/human/recruiter)
	...
	if(HAS_TRAIT(recruit, TRAIT_NOBLE))
		if(!HAS_TRAIT(recruit,TRAIT_OUTLANDER))
			recruiter.say("Своей великой властью я лишаю тебя, [uppertext(recruit.name)], ДВОРЯНСТВА!!")
			REMOVE_TRAIT(recruit, TRAIT_NOBLE, TRAIT_GENERIC)
			REMOVE_TRAIT(recruit, TRAIT_NOBLE, TRAIT_VIRTUE)
			return FALSE
		else
			to_chat(recruiter, span_warning("Не мне лишать таких дворянства!"))
			return FALSE
	recruiter.say("Своей великой властью я дарую тебе, [uppertext(recruit.name)], ДВОРЯНСТВО!!")
	ADD_TRAIT(recruit, TRAIT_NOBLE, TRAIT_GENERIC)
	REMOVE_TRAIT(recruit, TRAIT_OUTLANDER, ADVENTURER_TRAIT)
	return TRUE

/obj/effect/proc_holder/spell/self/convertrole/servant
	name = "Завербовать слугу"
	new_role = "Servant"
	overlay_state = "recruit_servant"
	recruitment_faction = "Servants"
	recruitment_message = "Служи короне, %RECRUIT!"
	accept_message = "За корону!"
	refuse_message = "Я отказываю."
	recharge_time = 100
...
/obj/effect/proc_holder/spell/self/convertrole/bog
	...
	recruitment_message = "Служи мне мечом, %RECRUIT!"
	accept_message = "За вас!"
	refuse_message = "Я отказываю."
