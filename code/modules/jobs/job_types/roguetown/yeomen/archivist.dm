/datum/job/roguetown/archivist
	title = "Archivist"
	tutorial = "Архивисты - хранители знаний, что сейчас так ценят в мире. Ведя войны сотни лет люди наконец поняли, что настоящие победы возможно получить лишь ЗНАЯ, как их получить. Ошибки прошлого идеально этому учат, однако не лишь древние войны представляют для тебя интерес. Многим людям нужны знания и твой долг им в этом помочь. Пиши книги, веди интеллектуальные беседы и оставляй в мировой памяти то, что может помочь твоим правнукам и их детям."
	flag = ARCHIVIST
	...
	job_traits = list(TRAIT_ARCYNE_T2, TRAIT_MAGEARMOR, TRAIT_INTELLECTUAL, TRAIT_SEEPRICES_SHITTY, TRAIT_GOODWRITER, TRAIT_TALENTED_ALCHEMIST, TRAIT_NOBLE)
...
/datum/outfit/job/archivist/basic/pre_equip(mob/living/carbon/human/H)
	...
	if(should_wear_femme_clothes(H))
		armor = /obj/item/clothing/suit/roguetown/shirt/dress/alizeria/library
		...
		if(L == usr)
			...
			if(L in range(1, usr))
				...
				if(L.get_skill_level(item) > SKILL_LEVEL_EXPERT)
					...
					if((item in list(/datum/language/monfit, /datum/language/draconic, /datum/language/dwarvish, /datum/language/elvish, /datum/language/otavan, /datum/language/grenzelhoftian, /datum/language/hellspeak, /datum/language/etruscan, /datum/language/gronnic)))
